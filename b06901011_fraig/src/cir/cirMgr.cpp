...
CirMgr::readCircuit(const string& fileName)
{
   ...
   for (size_t i = 0; i < size_t(spec[0]+spec[3]+1); i++){
      gateID[i] = -1;
      CirGate* tmp = new AIG(0,0,NULL,false,NULL,false,NON);
		_totalGate[i] = tmp;
   }
   ...
   for(size_t i = 0; i < _AIG.size(); i++){
      for (size_t j = 0; j < 2; j++){
         ...
         if (_totalGate[array[i][j]]->getTypeStr() != "NON" ){
   ...
   for (size_t i = 0; i < _totalGate.size(); i++){
      if(_totalGate[i]->getTypeStr() != "NON" && _totalGate[i]->getTypeStr() != "PO") { _totalGate[i]->sortFanout();}
   }  
   ...
   /*for (size_t i = 0; i < _totalGate.size(); i++){
      if (_totalGate[i] != NULL){
         if (_totalGate[i]->getIsInDFS()) cerr << _totalGate[i]->getID() << "-> true" << endl;
         else cerr << _totalGate[i]->getID() << "-> false" << endl;
      }
...
CirMgr::printSummary() const
{
   ...
   cout << "------------------" << endl;
   cout << "  " << "Total" << setw(9) << (_piGate.size() + _poGate.size() + _AIG.size()) << endl;
}
...
CirMgr::printFloatGates() const
{
   ...
   vector<int> noused;  
   for (size_t i = 0; i < _totalGate.size(); i++){
      if(_totalGate[i]->getTypeStr() != "NON" &&_totalGate[i]->getTypeStr() != "PI" && _totalGate[i]->getTypeStr() != "CONST" && _totalGate[i]->getTypeStr() != "UNDEF"){
   ...
   for (size_t i = 0; i < _totalGate.size(); i++)
      if (_totalGate[i]->getTypeStr() != "NON" && _totalGate[i]->getTypeStr() != "PO" && _totalGate[i]->getTypeStr() != "CONST" && _totalGate[i]->getFanoutSize() == 0) noused.push_back(_totalGate[i]->getID());
