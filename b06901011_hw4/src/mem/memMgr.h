...
#define toSizeT(t)      (t%SIZE_T!=0)?(t/SIZE_T)*SIZE_T+SIZE_T:t  // TODO
//
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
      MemBlock<T>* _lastMemBlock = _activeBlock;
      MemBlock<T>* _nextMemBlock = _lastMemBlock -> _nextBlock; 
  //按照順序把memblock刪掉
      while(_nextMemBlock != NULL){
        delete _lastMemBlock;
        _lastMemBlock = _nextMemBlock;
        _nextMemBlock = _lastMemBlock -> _nextBlock; 
      }
      _activeBlock = _lastMemBlock;
      _activeBlock -> reset();
//reset _recycleList[]
      for (int i = 0; i < R_SIZE; i++){
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* _nextList1 = &_recycleList[m];
      MemRecycleList<T>* _nextList2 = _nextList1-> _nextList;

      while(_nextList2 != 0){
        if (_nextList1 -> _arrSize == n) return _nextList1;
        else {
          _nextList1 = _nextList2;
          _nextList2 = _nextList2 -> _nextList;
        }
      }
      if (_nextList1->_arrSize == n) return _nextList1;
      else {
        MemRecycleList<T>* _new = new MemRecycleList<T>(n);
        _nextList1 -> _nextList = _new;
        return _new;
      }
      ...
      if (t > _blockSize){
        ...
        //沒有recycleList
          ...
          //remainsize不夠
            ...
            MemBlock<T>* _new = new MemBlock<T> (_activeBlock, _blockSize);
            _activeBlock = _new;
            _activeBlock->getMem(t,ret);
            #ifdef MEM_DEBUG
   ...
   size_t getNumBlocks() const {
      ...
      size_t num = 1;
      MemBlock<T>* _lastmemblock = _activeBlock;
      while(_lastmemblock->_nextBlock != NULL){
        _lastmemblock = _lastmemblock -> _nextBlock;
        num++;
      }
      /*
      MemBlock<T>* _nextmemblock = _lastmemblock->_nextBlock;
      while(_nextmemblock != NULL){
        _lastmemblock = _nextmemblock;
        _nextmemblock = _nextmemblock->_nextBlock;
        num++;
      }*/
      return num;
   }
