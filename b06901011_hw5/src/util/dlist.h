...
public:
   ...
   void push_back(const T& x) {
      iterator it = end();
      DListNode<T>* tmp = new DListNode<T> (x, it._node->_prev, it._node);
   ...
   void pop_front() {
      ...
      if (empty()){}
         ...
         end()._node -> _next = it._node->_next;
         it._node->_next->_prev = end()._node;
         _head = _head -> _next;
         delete it._node;
   }
      }
   ...
   void clear() { for (iterator it = begin(); it != end(); it++) erase(it); }  // delete all nodes except for the dummy node
